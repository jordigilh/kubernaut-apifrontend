...
`, name, namespace, a2aTaskID, joinMode, userEmail, sharedRRName, rrNamespace)
	...
	It("TC-E2E-SESSION-JOIN-04: TTL cancel vs reconnect race", func() {
		ctx := context.Background()
		sess := "e2e-g19-ttl-race"
		DeferCleanup(func() {
			_, _ = kubectl(ctx, "delete", "investigationsession", sess, "-n", namespace, "--ignore-not-found")
		})
...
spec:
  ...
  remediationRequestRef:
    name: %s
    namespace: %s
`, sess, namespace, e2ePersonas["sre"].Email, sharedRRName, rrNamespace)
		...
		statusPatch := `{"status":{"phase":"Disconnected","message":"ttl race","connectionState":"Disconnected","disconnectedAt":"2020-01-01T00:00:00Z"}}`
		out2, err2 := kubectl(ctx, "patch", "investigationsession", sess, "-n", namespace,
			"--type=merge", "--subresource=status", "-p", statusPatch)
		Expect(err2).NotTo(HaveOccurred(), out2)

		done := make(chan string, 1)
		go func() {
			patch := `{"status":{"phase":"Active","message":"race reconnect","connectionState":"Connected"}}`
			_, _ = kubectl(ctx, "patch", "investigationsession", sess, "-n", namespace,
				"--type=merge", "--subresource=status", "-p", patch)
			done <- "reconnect"
		}()

		Eventually(func() string {
			ph, kerr := kubectl(ctx, "get", "investigationsession", sess, "-n", namespace,
				"-o", "jsonpath={.status.phase}")
			if kerr != nil {
				return ""
			}
			return ph
		}, 45*time.Second, 500*time.Millisecond).Should(BeElementOf("Active", "Cancelled", "Failed"))

		final, err := kubectl(ctx, "get", "investigationsession", sess, "-n", namespace,
			"-o", "jsonpath={.status.phase}")
		Expect(err).NotTo(HaveOccurred())
		Expect(final).To(BeElementOf("Active", "Cancelled", "Failed"))
		<-done
	})

	It("TC-E2E-SESSION-JOIN-05: Reconnect after pod restart", func() {
		ctx := context.Background()
		name := "e2e-g19-restart"
		DeferCleanup(func() {
			_, _ = kubectl(ctx, "delete", "investigationsession", name, "-n", namespace, "--ignore-not-found")
		})

		Expect(applyInvestigationSession(name, e2ePersonas["sre"].Email, "start", "task-g19-restart")).To(Succeed())
		...
		_, err = kubectl(ctx, "delete", "pod", pod, "-n", namespace, "--grace-period=15")
		Expect(err).NotTo(HaveOccurred())
		_, err = kubectl(ctx, "rollout", "status", "deployment/apifrontend", "-n", namespace, "--timeout=180s")
		Expect(err).NotTo(HaveOccurred())

		out, err := kubectl(ctx, "get", "investigationsession", name, "-n", namespace,
